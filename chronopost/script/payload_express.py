...
# Using BASE_PAYLOAD from standard but with specific modifications for Europe
from .payload_fr import BASE_PAYLOAD, CHRONO_RELAIS_PRODUCT

def build_payload_relais_europe(data=None):
    """
    Fonction dédiée pour Chrono Relais Europe.
    Similaire à Relais France mais avec override du pays destinataire.
    """
    payload = dict(p.split("=", 1) for p in BASE_PAYLOAD.split("&"))
    
    payload["product"] = CHRONO_RELAIS_PRODUCT
    payload["valeurproduct"] = "relais" # ou "relais_europe", mais le backend attend "relais" pour le product code
